...
public class GrabObject : MonoBehaviour
{
    private struct LaunchData
    {
        ...
        public LaunchData(Vector2 _stratingPos, Vector2 _startingHeigth, Vector2 _direction, float _distance)
        {
            process = true;
            direction = _direction;
            distance = _distance;
            currentDist = 0.0f;
            stratingPos = _stratingPos;
    ...
    [SerializeField] private float m_startCollideTime = 0.2f;
    ...
    {
        if (m_launchData.process && m_launchData.duration > m_startCollideTime)
        {
    ...
    public void Release(Vector2 _pos, Vector2? _direction = null, float _distance = 0.0f)
    {
        ...
        m_launchData = new LaunchData(_pos, (Vector2)m_object.localPosition - m_objectOffset, _direction ?? Vector2.zero, _distance);
    }
